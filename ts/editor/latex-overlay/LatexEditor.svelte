...
<script context="module" lang="ts">
    ...
    export let closeLatexEditor: (() => void) | null = null;

    const closeSignalStore = writable<boolean>(false, (set) => {
        closeLatexEditor = () => set(true);
        return () => (closeLatexEditor = null);
    });
</script>
...
<div class="latex-editor" class:light-theme={!$pageTheme.isDark}>
    <CodeMirror
        {code}
        {configuration}
        bind:api={codeMirror}
        on:change={({ detail: latexText }) => code.set(latexText)}
        on:blur
    />
</div>
